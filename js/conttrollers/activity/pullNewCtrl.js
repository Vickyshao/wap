define(['jweixin', 'js/module.js', 'jquery', 'ngdialog', 'SHR256'], function (wx, controllers, $, ngdialog, SHR256, weixin) {
    controllers.controller('pullNewCtrl', ['$scope', '$rootScope', 'resourceService', '$filter', '$state', 'ngDialog', '$location', 'md5', '$localStorage', '$stateParams', function ($scope, $rootScope, resourceService, $filter, $state, ngDialog, $location, md5, $localStorage, $stateParams) {
        ...
        if($rootScope.getUrlParam('toFrom') == 'zhihu002') {
            $scope.zhihu = true;
        }
        ...
        $scope.toReceive = function () {
            if($rootScope.fromNative) {
                ...
                if(!($scope.isLogin)){
                    $state.go('zhuce',{toFrom: $stateParams.toFrom});
                } else {
