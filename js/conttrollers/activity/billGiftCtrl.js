define(['jweixin', 'js/module.js', 'jquery', 'ngdialog', 'SHR256'], function (wx, controllers, $, ngdialog, SHR256, weixin) {
    controllers.controller('billGiftCtrl', ['$scope', '$rootScope', 'resourceService', '$filter', '$state', 'ngDialog', '$location', 'md5', '$localStorage', '$stateParams', function ($scope, $rootScope, resourceService, $filter, $state, ngDialog, $location, md5, $localStorage, $stateParams) {
        $filter('isPath')('billGiftSem');
        $('body').scrollTop(0);
        $scope.toFrom = $rootScope.getUrlParam('toFrom');
        if($scope.toFrom == '306yg003') {
            $rootScope.title = '云柜用户专享送话费';
        } else {
            $rootScope.title = '新用户专享送话费';
        }
        $scope.login = {};
        $scope.isShowImgCode = false;
