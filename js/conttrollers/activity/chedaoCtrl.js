define(['jweixin', 'js/module.js', 'jquery', 'ngdialog', 'SHR256'], function (wx, controllers, $, ngdialog, SHR256, weixin) {
    controllers.controller('chedaoCtrl', ['$scope', '$rootScope', 'resourceService', '$filter', '$state', 'ngDialog', '$location', 'md5', '$localStorage', '$stateParams', function ($scope, $rootScope, resourceService, $filter, $state, ngDialog, $location, md5, $localStorage, $stateParams) {
        $filter('isPath')('chedao');

        $('body').scrollTop(0);
        $rootScope.title = '车到加油专题';
        $scope.login = {};
        $scope.isLogin = $filter('isRegister')().register;

        if($rootScope.fromNative){
            $('.chedao').css('padding-top','0')
        }
        ...
        $scope.getyzm = function (tegForm) {
            if ($scope.isSubMin == true) {if (tegForm.picCode.$invalid) {
