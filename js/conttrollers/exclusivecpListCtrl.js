define(['jweixin', 'js/module.js', 'ngdialog'], function (wx, controllers, ngdialog) {
    controllers.controller('exclusivecplistCtrl', function ($scope, $rootScope, resourceService, $filter, $state, $localStorage, $timeout) {
        $rootScope.title = "专属产品";
        delete $localStorage.coupon;
        $filter('isPath')('exclusivecplist');
        var isLoad = true;
        var pageOn = 1;
        $scope.cpList = [];
        resourceService.queryPost($scope, $filter('getUrl')('cplist'), { type: 2 , productUseType:2}, { name: '产品列表' });
